...
type VerificationKeyResult = Result<VerificationKey, Asn1Error>;
type RsaVerificationKeyResult = Result<RsaVerificationKey, Asn1Error>;

// Long form length
// * Must be used when the length is 128 or greater
// * XXX: We do not accept lengths greater than 32-bit.
fn long_length(b: &ByteSeq, offset: usize, len: usize) -> UsizeResult {
    if len > 4 {
        UsizeResult::Err(Asn1Error::SequenceTooLong)
    } else {
        UsizeResult::Ok(
            declassify_u32_from_U32(U32_from_be_bytes(U32Word::from_slice(b, offset, len)))
                as usize
                >> ((4 - len) * 8),
        )
    }
...
fn length_length(b: &ByteSeq, offset: usize) -> usize {
    if U8::declassify(b[offset]) >> 7 == 1u8 {
        // Only in this case we have a length length.
        declassify_usize_from_U8(b[offset] & U8(0x7fu8))
    } else {
        0
    }
}
...
fn short_length(b: &ByteSeq, offset: usize) -> UsizeResult {
    if U8::declassify(b[offset]) & 0x80u8 == 0u8 {
        UsizeResult::Ok(declassify_usize_from_U8(b[offset] & U8(0x7fu8)))
    } else {
        UsizeResult::Err(Asn1Error::Error)
    }
}
...
fn length(b: &ByteSeq, mut offset: usize) -> DoubleUsizeResult {
    if U8::declassify(b[offset]) & 0x80 == 0u8 {
...
fn read_sequence_header(b: &ByteSeq, mut offset: usize) -> UsizeResult {
...
fn check_tag(b: &ByteSeq, offset: usize, value: u8) -> Result<(), Asn1Error> {
    if U8::declassify(b[offset]) == value {
        Result::<(), Asn1Error>::Ok(())
    } else {
        Result::<(), Asn1Error>::Err(Asn1Error::InvalidTag)
    }
}
...
///
/// Returns the new offset into the bytes.
fn skip_sequence(b: &ByteSeq, mut offset: usize) -> UsizeResult {
    check_tag(b, offset, 0x30u8)?;
    offset = offset + 1;
...
/// We don't really care, just check that it's some valid structure and keep the
/// offset moving.
fn read_integer(b: &ByteSeq, mut offset: usize) -> UsizeResult {
...
type Spki = (SignatureScheme, CertificateKey);

pub fn x962_ec_public_key_oid() -> ByteSeq {
    byte_seq!(0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x02, 0x01)
}
pub fn ecdsa_secp256r1_sha256_oid() -> ByteSeq {
    byte_seq!(0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x03, 0x01, 0x07)
}
pub fn rsa_pkcs1_encryption_oid() -> ByteSeq {
    byte_seq!(0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01)
}
...
fn check_success(val: bool) -> Result<(), Asn1Error> {
    if val {
        ...
        Result::<(), Asn1Error>::Err(Asn1Error::Error)
    }
}

fn read_spki(cert: &ByteSeq, mut offset: usize) -> SpkiResult {
    ...
    if ec_oid.len() == oid_len {
        ...
        for i in 0..ec_oid.len() {
            let oid_byte_equal = U8::declassify(cert[oid_offset + i]) == U8::declassify(ec_oid[i]);
        ...
        if ec_pk_oid {
            ...
            for i in 0..ec_oid.len() {
                let oid_byte_equal =
                    U8::declassify(cert[oid_offset + i]) == U8::declassify(ec_oid[i]);
    ...
    if rsa_oid.len() == oid_len {
        ...
        for i in 0..rsa_oid.len() {
            let oid_byte_equal = U8::declassify(cert[oid_offset + i]) == U8::declassify(rsa_oid[i]);
    ...
    if U8::declassify(cert[offset]) == 0x00 {
    ...
    if ec_pk_oid && ecdsa_p256 {
        ...
        if rsa_pk_oid {
            ...
            SpkiResult::Err(Asn1Error::InvalidCertificate)
        }
    }
}
...
pub fn verification_key_from_cert(cert: &ByteSeq) -> SpkiResult {
...
pub fn ecdsa_public_key(cert: &ByteSeq, indices: CertificateKey) -> VerificationKeyResult {
    ...
    VerificationKeyResult::Ok(cert.slice(offset, len))
}

pub fn rsa_public_key(cert: &ByteSeq, indices: CertificateKey) -> RsaVerificationKeyResult {
...
pub fn cert_public_key(
    cert: &ByteSeq,
    signature_scheme: &SignatureScheme,
    indices: CertificateKey,
) -> PkResult {
    match signature_scheme {
        SignatureScheme::ED25519 => PkResult::Err(Asn1Error::UnsupportedAlgorithm),
        SignatureScheme::EcdsaSecp256r1Sha256 => {
            let pk = ecdsa_public_key(cert, indices)?;
            PkResult::Ok(PublicVerificationKey::EcDsa(pk))
        }
        SignatureScheme::RsaPssRsaSha256 => {
            let pk = rsa_public_key(cert, indices)?;
...
mod unit_test {
    ...
    fn test(cert: &ByteSeq) {
        let (signature_scheme, key) = verification_key_from_cert(&cert).unwrap();
        let pk = cert_public_key(cert, &signature_scheme, key)
            .expect("Error reading public key from cert");
        println!("Got pk {:?}", pk);
    }

    #[test]
    fn ecdsa_cert() {
        let cert = ByteSeq::from_public_slice(&CLOUDFLARE_COM_DER);
        test(&cert);
    }

    #[test]
    fn rsa_cert() {
        let cert = ByteSeq::from_public_slice(&GOO_GL_DER);
        test(&cert);
    }
    ...
    fn read_cert() {
        ...
        for file in files {
            ...
            test(&ByteSeq::from_public_slice(&bytes));
        }
    }
