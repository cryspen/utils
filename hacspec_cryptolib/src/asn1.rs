...
fn long_length(b: &ByteSeq, offset: usize, len: usize) -> UsizeResult {
    if len > 4 {
        UsizeResult::Err(ASN1_SEQUENCE_TOO_LONG)
    } else {
        UsizeResult::Ok(
...
fn check_tag(b: &ByteSeq, offset: usize, value: u8) -> Result<(), Asn1Error> {
    if U8::declassify(b[offset]) == value {
        ...
        Result::<(), Asn1Error>::Err(ASN1_INVALID_TAG)
    }
}
...
fn check_success(val: bool) -> Result<(), Asn1Error> {
    if val {
        ...
        Result::<(), Asn1Error>::Err(ASN1_ERROR)
    }
}
...
fn read_spki(cert: &ByteSeq, mut offset: usize) -> SpkiResult {
    ...
    if ec_pk_oid && ecdsa_p256 {
        ...
        if rsa_pk_oid {
            ...
            SpkiResult::Err(ASN1_INVALID_CERTIFICATE)
        }
    }
}
...
pub fn cert_public_key(cert: &ByteSeq, spki: &Spki) -> PkResult {
    match spki.0 {
        SignatureScheme::ED25519 => PkResult::Err(ASN1_UNSUPPORTED_ALGORITHM),
        SignatureScheme::EcdsaSecp256r1Sha256 => {
...
mod unit_test {
    ...
    fn test(cert: &ByteSeq) {
        let spki = verification_key_from_cert(&cert).unwrap();
    ...
    fn ecdsa_cert() {
        ...
        test(&cert);
        test(&ByteSeq::from_public_slice(&OTHER_ECDSA_P256_SHA256_CERT));
    }
